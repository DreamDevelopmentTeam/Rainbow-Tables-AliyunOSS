...
# 定义一个函数，用于保存数据和哈希值到阿里云OSS中
def save_data(data, hash_value, hash_function):
    # 构造阿里云OSS中的路径和文件名
    oss_path = save_path + hash_function + "/" + hash_value + "/"
    oss_file = oss_path + "data.json"

    # 检查阿里云OSS中是否已经存在相同的哈希值文件，如果存在，则读取其中的数据列表
    data_list = []
    if bucket.object_exists(oss_file):
        data_json = bucket.get_object(oss_file).read()
        data_list = json.loads(data_json)["data"]

    # 将新的数据添加到数据列表中，如果不存在重复的话
    if data not in data_list:
        data_list.append(data)

    # 将数据列表转换为JSON格式，并上传到阿里云OSS中，覆盖已有的文件
    data_json = json.dumps({"data": data_list})
    bucket.put_object(oss_file, data_json)
